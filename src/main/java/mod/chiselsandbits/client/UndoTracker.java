...
public class UndoTracker implements ICacheClearable
{
			...
			final VoxelBlobStateReference after )
	{
		...
		if ( pos != null && world != null && world.isClientSide && recording )
		{
			...
			if ( undoLevels.size() > ChiselsAndBits.getConfig().getClient().maxUndoLevel.get() )
			{
				undoLevels.remove( 0 );
			}
	...
	public void undo()
	{
		...
		{
            ClientSide.instance.getPlayer().sendSystemMessage( Component.translatable( "mod.chiselsandbits.result.nothing_to_undo" ) );
		}
	}
	...
	public void redo()
	{
		...
		{
			ClientSide.instance.getPlayer().sendSystemMessage( Component.translatable( "mod.chiselsandbits.result.nothing_to_redo" ) );
		}
	}
